...
public class CrearCotizacion extends AppCompatActivity {
    ...
    public void crearCotizacion() {
        try {
            if(txtNombreCoti.getText().toString().isEmpty()){
                ...
                cemento.setImagenCemento(cementoProducto.getImgUrl());
                cemento.setMarcaCemento(cementoProducto.getMarca());
                cemento.setTienda_idTienda(cubicar.getIdTienda());
                cemento.setRetiro(cementoProducto.getRetiro());
                cemento.setDespacho(cementoProducto.getDespacho());
                cemento.setDescripcionCemento(cementoProducto.getDescripcion());
                ...
                cemento.setPrecio(new BigDecimal(cementoPrice.getPrecio()));


                Call<Cemento> cementoCall = RetrofitBuilder.cementoService.crearCemento(cemento);
                cementoCall.enqueue(new Callback<Cemento>() {
                    @Override
                    public void onResponse(Call<Cemento> call, Response<Cemento> response) {
                        if (response.isSuccessful()) {
                            System.out.println("Cemento creado: ");
                            int idCemento = response.body().getIdCemento();
                            crearCubicacion(idCemento);
                        }
                    }

                    @Override
                    public void onFailure(Call<Cemento> call, Throwable t) {
                        Log.d("Error", t.getMessage());
                    }
                });
            }
    ...
    private void crearCubicacion(int idCemento) {
        try {
            ...
            cubicacion.setCantidadSacos(sacosAprox);
    ...
    public void guardarCotizacion(int idCemento, int idCubicacion) {
        ...
        try {
                ...
                detalleCotizacion.setCemento_idCemento(idCemento);
                System.out.println("ID Cubicacion " + idCubicacion);
                System.out.println("ID de usuario " + userSave);
                System.out.println("ID Cemento " + idCemento);
